...
Cross-platform CLI for monitor brightness control.
...
All displays (built-in + external) are unified under the same interface with consistent IDs: `builtin`, `1`, `2`, etc. Display lookup supports both ID and monitor name (case-insensitive).

### macOS (`src/macos.rs` + dark mode in `main.rs`)
...
### Windows (`src/windows.rs` + dark mode in `main.rs`)
...
### Linux (`src/linux.rs` + dark mode in `main.rs`)
...
Dark mode functions (`set_dark_mode`, `get_dark_mode`) live in `main.rs` behind `#[cfg(target_os)]` blocks, not in the platform modules. They use OS-specific CLI tools/APIs directly.
